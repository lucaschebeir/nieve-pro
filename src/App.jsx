...
<Btn variant="gold" size="sm" disabled={pendingPast===0&&pendingARS===0} onClick={()=>onSettle(viewStaff)}>✓ Liquidar</Btn>
  ...
  <div style={{display:"flex",flexDirection:"column",gap:5}}>
    ...
    {myClasses.filter(c=>isPendingFor(c,viewStaff.id)).length===0&&extraCommissions.filter(e=>e.staffId===viewStaff.id&&!e.isSettled).length===0&&<Empty text="Sin pendientes"/>}
            ...
            {staffTab==="history"&&(()=>{
              const settled=myClasses.filter(c=>!isPendingFor(c,viewStaff.id)&&(!histFrom||c.classDate>=histFrom)&&(!histTo||c.classDate<=histTo));
              const settledExtras=extraCommissions.filter(e=>e.staffId===viewStaff.id&&e.isSettled&&(!histFrom||e.date>=histFrom)&&(!histTo||e.date<=histTo));
              return(
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  {settled.length===0?<Empty text="Sin historial"/>:<div style={{display:"flex",flexDirection:"column",gap:5}}>{settled.map(c=>{const isInstrH=c.instructorId===viewStaff.id&&(c.scenario==="instructor_only"||c.scenario==="seller_and_instructor");const isARSH=c.currency==="ARS";const earnH=isInstrH?c.instructorEarning:(isARSH?null:c.sellerCommission);return(<div key={c.id} style={{background:T.surface,borderRadius:8,padding:"10px 14px",fontSize:12,borderLeft:`3px solid ${T.muted}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}><span style={{color:T.textDim}}>{fmtDate(c.classDate)}</span><span style={{fontWeight:700,flex:1,paddingLeft:8}}>{c.clientName}</span><PayBadge status={c.paymentStatus}/>{isARSH&&!isInstrH?<span style={{fontFamily:"monospace",color:T.purple,fontWeight:700}}>→ ${(c.sellerCommission||0).toLocaleString("es-AR")} ARS</span>:<span style={{fontFamily:"monospace",color:T.green,fontWeight:700}}>→ {fmt(earnH)}</span>}</div>{c.classTypeName&&<div style={{color:T.muted,fontSize:11,marginTop:3}}>{c.classTypeName}</div>}{c.notes&&<div style={{color:T.muted,fontSize:11,marginTop:2}}>{c.notes}</div>}</div>);})}</div>}
                  {settledExtras.length>0&&(
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      <div style={{fontSize:11,fontWeight:700,color:T.teal,textTransform:"uppercase",marginTop:4}}>Comisiones adicionales liquidadas</div>
                      {settledExtras.map(e=>(
                        <div key={e.id} style={{background:T.surface,borderRadius:8,padding:"10px 14px",fontSize:12,borderLeft:`3px solid ${T.teal}`}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
                            <span style={{color:T.textDim}}>{fmtDate(e.date)}</span>
